...
export default function DashboardShell({ hideHomeLink = false, children }) {
    ...
    <div className="min-h-screen flex flex-col">
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link to={user ? `/${user.role.key}` : '/'} className="flex items-center gap-2">
      ...
      <footer className="border-t border-slate-200 py-4 text-center text-xs text-slate-500">
        School Platform · v0.10 (PWA) · <Link to="/about" className="hover:underline">About</Link> · <Link to="/contact" className="hover:underline">Contact</Link>
      </footer>
      <InstallPrompt />
    </div>
  );
